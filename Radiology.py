...
for i1 in range(0, K):
    obj_function.append(cycle[i1] - 10*rhos[i1])
file1 = open("Output_Radiology.txt", "w")
file1.write("Run\t\tObjective function\n")
for i1 in range(0, K):
    file1.write("{}\t\t{}\n".format(i1, obj_function[i1]))
#var_obj = statistics.stdev(obj_function)
file1.write("\n\n")
#file1.write("Standard deviation is: {}".format(var_obj))
index = []
for i1 in range(1, N + 1):
    index.append(i1)
# get running avg objective function
running_avg_objective = []
for i1 in range(0, N):
    obj = running_avg_cycle[i1] - 10*running_avg_rho[i1]
    running_avg_objective.append(obj)
sd_obj = statistics.stdev(cycle_times)
file1.write("The standard deviation is: {}".format(sd_obj))
interval_lower = running_avg_objective[N - 1] * 0.985
interval_upper = running_avg_objective[N - 1] * 1.015
plt.hlines(interval_lower, 1, 1000, colors='red')
plt.hlines(interval_upper, 1, 1000, colors='red')
plt.plot(index, running_avg_objective)
plt.show()
performance_measure_final = statistics.mean(performance_measure_batch)
print(performance_measure_final)
